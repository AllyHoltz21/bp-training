(() => {
  ...
  // Soft breath-like "phhh" of someone blowing air into the cuff — quiet.
  function playPumpSound() {
    ...
    bp.type = 'bandpass';
    bp.Q.value = 1.2; // narrower band → "breath through lips" tone
    bp.frequency.setValueAtTime(650, t);
    bp.frequency.linearRampToValueAtTime(430, t + 0.35); // breath tapering off
    const g = audioCtx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.linearRampToValueAtTime(0.09, t + 0.08);   // soft breath in
    g.gain.linearRampToValueAtTime(0.0001, t + 0.42); // breath out
    src.connect(bp);
    bp.connect(g);
    g.connect(masterGain);
    src.start(t);
  ...
  function startReleaseHiss() {
    ...
    releaseGain.gain.setTargetAtTime(0.10, audioCtx.currentTime, 0.06);
    src.connect(lp);
